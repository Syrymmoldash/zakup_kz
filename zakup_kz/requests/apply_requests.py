...
class ApplyRequests():
    ...
    def parse_search_page(self, response):
        ...
        if (status in ['Опубликован (прием заявок)', 'Опубликовано (прием заявок)']) or (status == 'Опубликован' and self.force_apply):
            ...
            # if self.remove_previous:
                # self.detect_previous_app(self.get_token_scrapy(response), True)

            self.detect_previous_app(self.get_token_scrapy(response), self.remove_previous)
    ...
    def detect_previous_app(self, token, remove=False):
        cookies = {
            'show_filter_app': 'block',
            'ci_session': token
        }
